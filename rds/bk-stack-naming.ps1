...
$Broker = $LambdaInput.ResourceProperties.Broker
$Region = $LambdaInput.ResourceProperties.Region
$SslSecurityGroup = $LambdaInput.ResourceProperties.SslSecurityGroup

try {
    if ($LambdaInput.RequestType -eq 'Create') {        
        # Broker network interface tags
        $SubnetName = ((Get-EC2Subnet -SubnetId ((Get-EC2Instance -InstanceId $Broker -Region $Region).Instances.SubnetId) -Region $Region).Tags | Where-Object {$_.Key -eq 'Name'}).Value
        $BrokerName = ((Get-EC2Instance -InstanceId $Broker -Region $Region).Instances.Tags | Where-Object {$_.Key -eq 'Name'} | Select-Object -ExpandProperty Value)
        $NetworkInterface = Get-EC2NetworkInterface -Region $Region | Where-Object {$_.Attachment.InstanceId -eq $Broker}
        $NetworkInterfaceName = '{0}-{1}-{2}-{3}-{4}-eni01' -f $CustomerAbbreviation, $SubnetName.Split('-')[1], $SubnetName.Split('-')[3], $SubnetName.Split('-')[4], $BrokerName.Split('-')[-1].ToLower()
        ...
        $Description = 'Interface eth{0} for EC2 instance {1} ({2})' -f $NetworkInterface.Attachment.DeviceIndex, $Broker, $BrokerName.ToLower()
        ...
        # Broker volume tags
        $Volume = Get-EC2Volume -Filter @{Name = 'attachment.instance-id'; Values = $Broker} -Region $Region
        $VolumeName = '{0}-{1}-{2}-vol01' -f $CustomerAbbreviation, $SubnetName.Split('-')[1], $BrokerName.Split('-')[-1].ToLower()
        ...
        $SecurityGroupName = '{0}-{1}-vpc01-sg02' -f $CustomerAbbreviation, $RegionCode
        New-EC2Tag -Resource $SslSecurityGroup -Tag @{Key = 'Name' ; Value = $SecurityGroupName} -Region $Region
